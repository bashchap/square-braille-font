#!/usr/bin/env python3
"""3D starfield fly-through on the PUA Square Braille virtual framebuffer."""
...
def main():
    ...
    frame = 0
    sys.stdout.write("\x1b]0;PUA Square Braille — Starfield\x07\x1b[?1049h\x1b[2J\x1b[H\x1b[?25l")
    sys.stdout.flush()
    try:
        while args.frames == 0 or frame < args.frames:
            ...
            for star in stars:
                ...
                if star.z <= 0.035:
                    star.reset()
                x, y = project(star, size.columns * 2, size.lines * 4)
                if x < -3 or x >= size.columns * 2 + 3 or y < -3 or y >= size.lines * 4 + 3:
                    star.reset()
