...
def main():
    ...
    points = triangle_pixels(size.columns * 2, size.lines * 4)
    colors = cell_colors(points)
    masks = [[0] * size.columns for _ in range(size.lines)]
    interval = 1.0 / args.pps
    start = time.monotonic()
    sys.stdout.write("\x1b]0;PUA Square Braille — Filled 180x120 RGB Triangle\x07"
    ...
    try:
        for count, ((x, y), _) in enumerate(points, 1):
            ...
            glyph = chr(PUA_START + masks[cell_y][cell_x])
            sys.stdout.write(
