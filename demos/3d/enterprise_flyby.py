#!/usr/bin/env python3
"""High-detail, depth-buffered TOS Enterprise fly-around for PUA Square Braille.
...
addresses a 2 x 4 array of square virtual pixels in U+E000..U+E0FF.
"""
...
PUA_START = 0xE000
DOT_WEIGHTS = np.array(((1, 8), (2, 16), (4, 32), (64, 128)), dtype=np.uint16)
MATERIALS = np.array((
    (132, 158, 166),  # 0 hull
...
def image_to_terminal(rgb, quantization=16):
    ...
    rows, columns = height // 4, width // 2
    rgb = rgb[:rows * 4, :columns * 2]
    blocks = rgb.reshape(rows, 4, columns, 2, 3).transpose(0, 2, 1, 3, 4)
    occupied = np.any(blocks != 0, axis=4)
    masks = np.sum(occupied * DOT_WEIGHTS[None, None, :, :], axis=(2, 3)).astype(np.uint8)
    luminance = blocks[..., 0].astype(np.uint16) * 3 + blocks[..., 1].astype(np.uint16) * 6 + blocks[..., 2]
    luminance[~occupied] = 0
    flat_index = np.argmax(luminance.reshape(rows, columns, 8), axis=2)
    flat_colors = blocks.reshape(rows, columns, 8, 3)
...
def main():
    ...
    def draw(timeline):
        rgb, _depth, triangles = render_frame(mesh, columns * 2, rows * 4,
