...
Each resulting screen pixel is encoded through U+E000..U+E0FF.
...
def image_to_terminal(rgb, quantization=16):
    ...
    blocks = rgb[:rows * 4, :columns * 2].reshape(rows, 4, columns, 2, 3).transpose(0, 2, 1, 3, 4)
    ...
    light[~occupied] = 0
    pick = np.argmax(light.reshape(rows, columns, 8), axis=2)
    colors = np.take_along_axis(blocks.reshape(rows, columns, 8, 3), pick[..., None, None], axis=2)[:, :, 0]
...
def main():
    ...
    def make_frame(timeline):
        rgb, triangles, edges = render(mesh_file, columns * 2, rows * 4, timeline,
