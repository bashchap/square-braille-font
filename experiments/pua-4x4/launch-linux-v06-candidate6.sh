...
PROFILE_ID="pua4x4v06candidate6"
...
case "$MODE" in
    setup|shell|demo|motion|reference|triangle|probe|raster|box) ;;
    *) echo "Usage: $0 [setup|shell|demo|motion|reference|triangle|probe|raster|box]" >&2; exit 2 ;;
esac

"$SCRIPT_DIR/install-linux-v06-candidate6.sh"
python3 "$SCRIPT_DIR/verify_linux_v04_candidate_runtime.py" \
    --build-dir "$PACKAGE_DIR" \
    --alias "$PROFILE_NAME" \
    --part0-family "PUA 4x4 Part 0 v0.6 Candidate 6" \
    --part1-family "PUA 4x4 Part 1 v0.6 Candidate 6" \
    --part0-file PUA4x4Part0V06Candidate6.ttf \
    --part1-file PUA4x4Part1V06Candidate6.ttf \
    --output "$SCRIPT_DIR/output/audit/pua4x4-v0.6-candidate6-linux-runtime.json" \
    --layout-output "$SCRIPT_DIR/output/audit/pua4x4-v0.6-candidate6-pango-layout.json"
...
echo "Candidate 6 profile ready: $PROFILE_NAME at $FONT_SIZE pt"
echo "Supported: normal and enlarged MATE Terminal sizes."
echo "Known limit: the two smallest Ctrl-minus zoom levels can reveal horizontal seams."
test "$MODE" = setup && exit 0

terminal_args=(--disable-factory "--profile=$PROFILE_NAME" --maximize "--working-directory=$SCRIPT_DIR")
case "$MODE" in
    demo)
        terminal_args+=("--title=PUA 4x4 v0.6 RC1 — Mapping and Resolution Proof"
            "--command=python3 $SCRIPT_DIR/pua4x4_demo.py")
        ;;
    motion)
        terminal_args+=("--title=PUA 4x4 v0.6 RC1 — Curved Vortex"
            "--command=python3 $SCRIPT_DIR/pua4x4_motion_demo.py")
        ;;
    reference)
        terminal_args+=("--title=PUA 4x4 v0.6 RC1 — Reference Renderer"
            "--command=python3 $SCRIPT_DIR/pua4x4_reference_renderer.py")
        ;;
    triangle)
        terminal_args+=("--title=PUA4X4-CANDIDATE6-FOREGROUND-TRIANGLE"
            "--command=python3 $SCRIPT_DIR/demos4x4/triangle.py --pps 100000 --hold 180")
        ;;
    raster)
        terminal_args+=("--title=PUA4X4-CANDIDATE6-FOREGROUND-RASTER"
            "--command=python3 $SCRIPT_DIR/terminal_multicolour_raster_probe.py --candidate-label 'Candidate 6' --hold 180")
        ;;
    probe)
        terminal_args+=("--title=PUA4X4-CANDIDATE6-MAPPING-PROBE"
            "--command=python3 $SCRIPT_DIR/terminal_candidate_probe.py --hold 180 --output $SCRIPT_DIR/output/audit/terminal-v0.6-candidate6/probe-runtime.json")
        ;;
    box)
        fontplotter_root="${FONTPLOTTER_ROOT:-$HOME/dev/FontPlotter}"
        test -f "$fontplotter_root/demos/framebuffer-box-diagnostic.py"
        terminal_args+=("--title=PUA4X4-CANDIDATE6-BOX-GRID"
            "--command=python3 $fontplotter_root/demos/framebuffer-box-diagnostic.py --fps 8")
        ;;
    shell)
        terminal_args+=("--title=PUA 4x4 v0.6 Candidate 6 Shell")
        ;;
esac
